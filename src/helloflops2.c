...
// helloflops2
//
// A simple example that gets lots of Flops on
// Intel(r) Xeon Phi(tm) coprocessors
// using openmp to scale
//
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#include <sys/time.h>
// dtime –
// utility routine to returns the current wall clock time
double dtime() {
	double tseconds = 0.0;
...
#pragma omp parallel for private(j,k)
	...
	gflops = (double) (1.0e-9 * numthreads *
	LOOP_COUNT * MAXFLOPS_ITERS * FLOPSPERCALC);
	// elasped time
	ttime = tstop - tstart;
	//
	// Print the results
