...
void stencil9pt_base(REAL *finp, REAL *foutp, int width, int height,
...
// offsets from center pt.
			...
			int se = s + 1;
			for (x = 1; x < width - 1; x++) {
				fout[c] = diag * fin[nw] + diag * fin[ne] + diag * fin[sw]
						+ diag * fin[se] + next * fin[w] + next * fin[e]
						+ next * fin[n] + next * fin[s] + ctr * fin[c];
...
int main(int argc, char *argv[]) {
	...
	t_begin = dtime();
	stencil9pt_base(fin, fout, width, height, stenctr, stennext, stendiag,
			count);
	t_end = dtime();
#if DBG
